...
import logo from '../../../images/buttonImage/PBB.svg'
...
class pbb extends Component {
    constructor(){
        ...
        this.handleChangeWilayah = this.handleChangeWilayah.bind(this)
    ...
    handleSubmit(e){
        e.preventDefault();
        this.setState({loading:true, cekTagihan:''})
        const isLogin = JSON.parse(localStorage.getItem('isLogin'))
        const header={
            'X-Api-Key' : X_API_KEY
        }
        ...
        axios.post(API+PASCABAYAR_CEK, body, {headers:header})
          .then(res => {
            ...
            if(order.errNumber !== '0'){
              ...
              document.getElementById('form_detail_pbb').scrollIntoView({behavior:'smooth'})
            }
            console.log(order)
          })
          .catch(error => {
    ...
    handleChangeWilayah(e){
      this.setState({[e.target.name] : e.target.value})
      const wilayah = this.state.product.filter((wil) =>{
           return wil.name.indexOf(e.target.value.toUpperCase()) !== -1;
      }
      )
      this.setState({pdam:wilayah})
    }
    componentDidMount(){
        const isLogin = JSON.parse(localStorage.getItem('isLogin')) 
        if (isLogin){
    ...
    render() {
      if(this.state.redirect === true){
        ...
        const {resTagihan, pdam} = this.state
        return (
            <div>
                <div className=' border shadow p-3 mt-3 mb-3 new-radius' id="form_detail_pbb">
                    <div className="row mt-2">
                        <div className='col-sm-12'>
                            <h3 style={{color:'#2E7D32',marginTop:'10px',marginBottom:'10px'}} >Pajak Daerah</h3>
                        </div>
                    </div>
                    <div className='row'>
                        ...
                        <div className='col-sm mt-2' >
                          {/* {console.log('code',this.state.code)} */}
                            <GreenTextField label="Masukkan Nama Wilayah" style={{width:'100%'}} 
                                name="code" 
                          ...
                          />
                            <div className="dropdown-menu w-100 scrol-pdam rounded-0 p-0">
                              {pdam.map(pdam=>(
                                <p className='menu-pdam' onClick={() => this.handleChangeCode(pdam.code)} key={pdam.code}>{pdam.name}</p>
                              ))}
                            </div>
                        </div>
                    </div>
                ...
                <div className='border shadow p-3 mt-3 mb-3 new-radius'>
                    ...
                    <div className='row mt-3 ml-0'>
                        <button style={style.btGreen}  data-toggle="modal" data-target="#pbb"  className='btn rounded-0'>PROSES</button>
                    </div>
                </div>
                } 

                 {/* <!-- Modal --> */}
                 <div className="modal fade" id="pbb" data-backdrop="static" tabIndex="-1" role="dialog" aria-labelledby="pbbLabel" aria-hidden="true">
...
export default connect(mapStateToProps, mapDispatchToProps)(pbb)
const style = {
    btGreen:{
        fontSize:'14px',
        color:'#FFFFFF',
