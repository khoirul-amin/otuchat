...
import {API,PULSA_PRODUCT,APLUS,X_API_KEY,PULSA_PREFIX,ORDER_PRODUCT} from '../../../api/index'
...
class Pulsa extends Component {
    ...
    handleChangeNomor(e){
        ...
        if(!pref[0] || nomor === ''){
            ...
            const pulsa = this.state.product.filter(
                (p) => {
                    return p.name.indexOf(pref[0].provider.toUpperCase()) !== -1;
                }
            )
            this.setState({productFilter: pulsa, tipe:pref[0].provider})
        }
        
        // const image = this.props.images
        if (!pref[0]){
            this.setState({logo : ''})
        }else if (pref[0].provider === 'Indosat'){
            this.setState({logo : imagePrefix.indosat})
        }else if(pref[0].provider === 'Three'){
            this.setState({logo: imagePrefix.three})
        }else if(pref[0].provider === 'Smart'){
            this.setState({logo: imagePrefix.smartfren})
        }else if(pref[0].provider === 'Axis'){
            this.setState({logo: imagePrefix.axis})
        }else if(pref[0].provider === 'Telkomsel'){
            this.setState({logo: imagePrefix.telkomsel})
        }else if(pref[0].provider === 'XL'){
    ...
    componentDidMount(){
        ...
        if (isLogin){
            ...
            axios.post(API+PULSA_PRODUCT, body, {headers:header} )
            .then(res => {
            ...
            axios.post(API+PULSA_PREFIX, body, {headers:header} )
            .then(res => {
    ...
    render() {
        ...
        let pulsa = this.state.productFilter
        return (
            <div>
                <div className='border shadow p-3 mt-3 mb-3 new-radius'>
                    <div className="row mt-2">
                        <div className='col-sm-12'>
                            <h3 style={{color:'#2E7D32',marginTop:'10px',marginBottom:'10px'}} >Pembelian Pulsa</h3>
                        </div>
                    </div>
                    <div className='row'>
                                ...
                                <div>
                                    ...
                                    <div>
                                        ...
                                        <div className="dropdown-menu scrol-pdam rounded-0 p-0" >
                                            {pulsa.map(pulsa=>(
                                                <div className='row m-0 menu-pdam' key={pulsa.code} style={{fontSize:'12pt'}} onClick={() => this.handleChange(pulsa.code,pulsa.name)}>
                                                    <div className='col m-0 p-0'>
                                                        <p className="mb-0">{pulsa.name}<br/><span style={{fontSize:'10pt'}}>+ Point {pulsa.ep}</span></p>
                                                    </div>
                                                    <div>
                                                        <p className="mb-0" style={{color:'#3A9460'}}>Rp {convertCurrency(pulsa.price)}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                    ...
                    <div className='row mt-5'>
                        <div className='col-sm-3'>
                            ...
                            :
                                <div align="center" data-toggle="modal" data-target="#pulsa" className='bt-green'> Beli</div>
                            }
                        </div>
                    </div>
                {/* <!-- Modal --> */}
                <div className="modal fade" id="pulsa" data-backdrop="static" tabIndex="-1" role="dialog" aria-labelledby="pulsaLabel" aria-hidden="true">
                        ...
                        <div className="modal-content border-0 bg-transparent">
                                ...
                                <div className="custom-modal-body" style={{borderBottom:'5px solid #CDF0DC'}}>
                                    <table className="w-100">
                                        <tbody>
                                            <tr>
                                                <td className="pt-4 pb-4 w-50"> <span style={{opacity:'60%'}}>Pulsa</span> <br/> {this.state.name}</td>
...
export default connect(null, mapDispatchToProps)(Pulsa)
