...
import {API,TELEPHONE_PRODUCT,APLUS,X_API_KEY,TELEPHONE_PREFIX,ORDER_PRODUCT} from '../../../api/index'
...
class Telephone extends Component {
    ...
    handleChangeNomor(e){
        ...
        if(!pref[0] || nomor === ''){
            ...
            const telephone = this.state.product.filter(
                (p) => {
                    return p.provider.indexOf(pref[0].provider.toUpperCase()) !== -1;
                }
            )
            this.setState({productFilter: telephone, tipe:pref[0].provider})
        }
        // console.log(pref[0].provider)
        if (!pref[0]){
            this.setState({logo : ''})
        }else if (pref[0].provider === 'ISAT TELPON'){
            this.setState({logo : imagePrefix.indosat})
        }else if(pref[0].provider === 'TRI TELPON'){
            this.setState({logo: imagePrefix.three})
        }else if(pref[0].provider === 'SMARTFREN TELPON'){
            this.setState({logo: imagePrefix.smartfren})
        }else if(pref[0].provider === 'AXIS TELPON'){
            this.setState({logo: imagePrefix.axis})
        }else if(pref[0].provider === 'TSEL TELPON'){
            this.setState({logo: imagePrefix.telkomsel})
        }else if(pref[0].provider === 'XL TELPON'){
    ...
    componentDidMount(){
        ...
        if (isLogin){
            ...
            axios.post(API+TELEPHONE_PRODUCT, body, {headers:header} )
            .then(res => {
            ...
            axios.post(API+TELEPHONE_PREFIX, body, {headers:header} )
            .then(res => {
            ...
            <div className=' border shadow p-3 mt-3 mb-3 new-radius'>
                <div className="row mt-2">
                    <div className='col-sm-12'>
                        <h3 style={{color:'#2E7D32',marginTop:'10px',marginBottom:'10px'}} >Pembelian Paket Telephone</h3>
                    </div>
                </div>
                <div className='row'>
                    ...
                    <div className='row mt-5'>
                        <div className='col-sm-3'>
                            ...
                            :
                                <div align="center" data-toggle="modal" data-target="#telephone" className='bt-green'> Beli</div>
                            }
                        </div>
                    </div>

                {/* <!-- Modal --> */}
                <div className="modal fade" id="telephone" data-backdrop="static" tabIndex="-1" role="dialog" aria-labelledby="telephoneLabel" aria-hidden="true">
                        ...
                        <div className="modal-content border-0 bg-transparent">
                            ...
                            <div className="custom-modal-body" style={{borderBottom:'5px solid #CDF0DC'}}>
                                <table className="w-100">
                                    <tbody>
                                        <tr>
                                            <td className="pt-4 pb-4 w-50"> <span style={{opacity:'60%'}}>Telephone</span> <br/> {this.state.name}</td>
...
export default connect(null, mapDispatchToProps)(Telephone)
